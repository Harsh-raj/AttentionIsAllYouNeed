...
class Train:
    ...
    def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=2):
        ...
        with torch.no_grad():
            for batch in validation_ds:
                ...
                model_out = Train.greedy_decode(model, encoder_input, encoder_mask, tokenizer_src, tokenizer_tgt, max_len, device)
    ...
    def get_or_build_tokenizer(config, ds, lang):
        ...
        if not Path.exists(tokenizer_path):
            ...
            tokenizer.train_from_iterator(Train.get_all_sentences(ds, lang), trainer=trainer)
    ...
    def get_ds(config):
        ...
        tokenizer_src = Train.get_or_build_tokenizer(config, ds_raw, config['lang_src'])
        tokenizer_tgt = Train.get_or_build_tokenizer(config, ds_raw, config['lang_tgt'])
    ...
    def train_model(config):
        ...
        model = Train.get_model(config, tokenizer_src.get_vocab_size(), tokenizer_tgt.get_vocab_size()).to(device)
...
if __name__ == '__main__':
    config = Config.load('config.yaml')  # Adjust as per actual config loading method
