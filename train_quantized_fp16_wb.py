...
class TrainWB:
  ...
  def get_ds(config):
      ...
      ds_raw = load_dataset('wmt14', f"{config['lang_src']}-{config['lang_tgt']}", split='train')
  ...
  def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # Save the model every 20,000 steps
            if global_step % 20000 == 0:
                model_filename = Config.get_weights_file_path(config, f"step_{global_step}.pth")
                torch.save({
                    'epoch': epoch,
